...
def test_dict_SpecDetector():
    detector_a = we_abst.SpecDetector('freq', detector_location=[0.0, 0.0, 1.0],
                                              ...
                                              interaction_filter=[{1: [-1], 2: [1], 3: [1]}]) #, {1: [-1], 2: [1], 3: [1]}
    dd = {'detection_method': 'freq', 'detector_location': [0.0, 0.0, 1.0], 'detection_polarization': [0.0, 0.0, 1.0], 
          ...
          'interaction_filter': [{1: [-1], 2: [1], 3: [1]}], 'ignore_collinear': True}
    assert asdict(detector_a) == dd
    assert asdict(detector_a)['interaction_filter'] == [{1: [-1], 2: [1], 3: [1]}]
    assert asdict(detector_a)['interaction_filter'] == detector_a.interaction_filter
