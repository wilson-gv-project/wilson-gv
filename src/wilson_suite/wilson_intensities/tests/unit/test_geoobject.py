...
"""
import pytest
from ...amplitudes.spectrum_composition import ResLocGeoObject

def test_point_creation():
    point = ResLocGeoObject({'A': 1864.0, 'B': 900.0})
...
def test_line_creation():
    line = ResLocGeoObject({'A': 1864.0, 'B': 'all'})
    assert line.is_line()
    assert not line.is_point()
    assert line.dimensionality == 1

def test_plane_creation():
    plane = ResLocGeoObject({'A': 'all', 'B': 'all', 'C': 1200.0})
    assert plane.is_plane()
    assert plane.dimensionality == 2
    assert plane['C'] == 1200.0

def test_hashable():
    point1 = ResLocGeoObject({'A': 1864.0, 'B': 900.0})
    point2 = ResLocGeoObject({'A': 1864.0, 'B': 900.0})
    point3 = ResLocGeoObject({'A': 1864.0, 'B': 'all'})
...
def test_ordering_invariance():
    obj1 = ResLocGeoObject({'A': 1.0, 'B': 2.0})
    obj2 = ResLocGeoObject({'B': 2.0, 'A': 1.0})
    assert obj1 == obj2
    assert hash(obj1) == hash(obj2)

def test_invalid_access():
    point = ResLocGeoObject({'A': 1.0, 'B': 2.0})
