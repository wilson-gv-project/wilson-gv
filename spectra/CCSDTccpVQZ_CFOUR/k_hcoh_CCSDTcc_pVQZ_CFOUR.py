...
def one_spectrum_fig(el, mech, region=None, gamma_rc=gamma_rc):
    ...
    name=f'./svgs/CCSDT_el{str(el)[0]}_mech{str(mech)[0]}_w1mw2{str(w1mw2)[0]}_log10{str(log10)[0]}_gamma{gamma_str}_x{str(int(start1))}t{str(int(stop1))}_y{str(int(start2))}t{str(int(stop2))}_step{step_str}pureD.svg'
    ...
    setup.plot2Dmatplotlib(Z, w1mw2=w1mw2, nametuple=(name, __file__, "CCSDT/cc-pVQZ"), Gamma=gamma, el=el, mech=mech, dpi=200, log10=log10)
    end_time = time.time()
